<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="UTF-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1.0">

		<link rel="stylesheet" href="public/css/bootstrap-utilities.rtl.min.css">
		<link rel="stylesheet" href="public/css/bootstrap-reboot.rtl.min.css">
		<link rel="stylesheet" href="public/css/bootstrap-utilities.min.css">
		<link rel="stylesheet" href="public/css/bootstrap-grid.rtl.min.css">
		<link rel="stylesheet" href="public/css/bootstrap-reboot.min.css">
		<link rel="stylesheet" href="public/css/bootstrap-grid.min.css">
		<link rel="stylesheet" href="public/css/bootstrap.rtl.min.css">
		<link rel="stylesheet" href="public/css/bootstrap.min.css">
		<link rel="stylesheet" href="public/css/style.css">

		<link rel="shortcut icon" href="public/image/favicon.ico" type="image/x-icon">
		
		<title>ilion - localhost</title>
	</head>
	<body class="">
		<div class="container">

			<label class="switch">
				<input type="checkbox" id="dm">
				<span id="slider" class="slider round"></span>
			</label>

			<div class="m-5">
				<div class="row justify-content-center">
					<div class="col text-center">
						<a href="index.php">
							<img src="public/image/logoilion.png" id="logoiliontop" alt="ilion Tecnologia" width="250px">
						</a>
					</div>
				</div>

				<div class="list-group m-5" id="list">
				</div>
			</div>

			<div class="footer">
				<footer class="py-3">
					<ul id="linef" class="nav justify-content-center border-bottom pb-3 mb-3">
						<li class="nav-item"><a href="/" class="nav-link px-2 text-muted">Home</a></li>
					</ul>
					<p class="text-center text-muted">
						<a href="#">
							© <?=date('Y');?> Ilion tecnologia, powered by - <img src="public/image/logotipoilion.png" id="logoilionbottom" alt="ilion icon">
						</a>
					</p>
				</footer>
			</div>
		</div>
		
		<script src="public/js/bootstrap.bundle.min.js"></script>
		<script src="public/js/bootstrap.min.js"></script>
		<script src="public/js/open.js"></script>